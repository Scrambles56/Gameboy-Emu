...
public class Mbc3Cartridge : LoadedCartridge
{
    ...
    public Mbc3Cartridge(ILogger logger, byte[] data) : base(logger, data)
    {
        ...
        {
            RomBanks[i] = new ROM(0x4000, 0x0000, data[(i * 0x4000)..((i + 1) * 0x4000)]);
        }
    ...
    public override void Write(ushort address, byte value)
    {
        ...
        else if (address.IsBetween(0xA000, 0xBFFF))
        {
            if (EnableRam)
            {
                if (RamBank01 != null)
                {
                    RamBank01.WriteByte(address, value);
                }
                else if (RamBank02 != null)
                {
                    RamBank02.WriteByte(address, value);
                }
                else if (RamBank03 != null)
                {
                    RamBank03.WriteByte(address, value);
                }
            }
        }
    }
    ...
    public override byte Read(ushort address)
    {
        ...
        if (address.IsBetween(0x4000, 0x7FFF))
        {
            try
            {
                return SelectedRomBank.ReadByte((ushort)(address-0x4000));
            }
            catch (Exception e)
            {
        ...
        if (address.IsBetween(0xA000, 0xBFFF))
        {
            if (EnableRam)
            {
                if (RamBank01 != null)
                {
                    return RamBank01.ReadByte(address);
                }
                if (RamBank02 != null)
                {
                    return RamBank02.ReadByte(address);
                }
                if (RamBank03 != null)
                {
                    return RamBank03.ReadByte(address);
                }
            }
            else
            {
